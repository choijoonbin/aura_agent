...
def _render_plotly(fig: go.Figure, **kwargs: Any) -> None:
    """Streamlit 버전 호환: width='stretch' 미지원 버전은 use_container_width로 fallback."""
    try:
...
def _render_dataframe(data: Any, **kwargs: Any) -> None:
    """Streamlit 버전 호환: width='stretch' 미지원 버전은 use_container_width로 fallback."""
    try:
...
def _render_line_plot(frame: pd.DataFrame, cols: list[str], *, x_col: str = "date", height: int = 300, x_title: str | None = None, y_title: str | None = None, single_point_mode: str = "markers") -> None:
    ...
    _render_plotly(fig, config={"displayModeBar": False})
...
def _render_overview_tab(frame: pd.DataFrame) -> None:
    ...
    with stylable_container(
        ...
        if recent.empty:
            ...
            _render_dataframe(present, hide_index=True, height=400)
    ...
    with left:
        with stylable_container(
            ...
            if signal_df.empty:
                ...
                _render_dataframe(signal_present, hide_index=True, height=260)
    ...
    with right:
        with stylable_container(
            ...
            if frame.empty or frame["severity"].dropna().empty:
                ...
                _render_dataframe(sev_present, hide_index=True, height=220)
        ...
        with stylable_container(
            ...
            if frame.empty:
                ...
                _render_dataframe(lineage_present, hide_index=True, height=220)
