    ...
    with col_scenario:
        with stylable_container(key="demo_scenario_panel", css_styles=panel_css):
            ...
            with btn_col:
                ...
                if st.button("생성", key="demo_seed_btn", use_container_width=True, type="primary") and scenario:
        ...
        with stylable_container(key="demo_batch_panel", css_styles=panel_css):
            ...
            with mid_c:
                ...
                for k, v in grouped.items():
                    st.caption(f"{k}: {v}건")
                if st.button("시연 데이터 전체 삭제", use_container_width=True, key="demo_delete_all"):
