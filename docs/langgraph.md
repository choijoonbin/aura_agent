...
critic            tool_results 기반 과잉 주장·반례 검토
  │               legacy 결과 존재 여부, missing_fields, score 품질 신호 평가
  │               → recommend_hold 판정
  ▼
...
execute
  ...
  │
  ├──▶ budget_risk_probe            예산 초과 여부 및 금액 신호 검증
...
| `budget_risk_probe` | 예산 초과 여부 및 금액 신호 검증 | 아니오 | [L43](../agent/skills.py#L43) |
...
| 항목 | 방식 | 파일 |
|---|---|---|
| 메인 오케스트레이션 그래프 | `draw_mermaid()` → regex 파싱 → matplotlib PNG | [`ui/studio.py` L15](../ui/studio.py#L15) |
| 스킬 실행 흐름 그래프 | 직접 networkx 구성 → matplotlib PNG | [`ui/shared.py` L403](../ui/shared.py#L403) |
| 그래프 표시 | `st.image()` (브라우저 JS 불필요) | [`ui/shared.py` L423](../ui/shared.py#L423) |
