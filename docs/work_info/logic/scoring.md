...
- **Fidelity 보수 산정 고정**: `fidelity = min(rule_fidelity, llm_fidelity)`
  - `rule_fidelity = evidence_completeness * 100`
  - `llm_fidelity = LLM Judge grounding 점수(0~100)`

### 1) 1차: 데이터 모델/설정 확장 (무중단)
- 대상 파일:
  - `agent/output_models.py`
  - `agent/langgraph_nodes.py` (AgentState/TypedDict 확장)
  - `utils/config.py`
  - `.env.example`
- 작업 내용:
  ...
  - AgentState(또는 TypedDict) 확장 필드 반영
    - 점수/게이트: `rule_score`, `llm_score`, `final_score`, `verification_gate`
    - 신뢰도: `fidelity`, `rule_fidelity`, `llm_fidelity`
    - 안정성/관측: `fallback_used`, `fallback_reason`, `retry_count`, `latency_ms`
    - 메타: `version_meta`(scoring/rubric/prompt 버전)
  - 설정값 추가
...
- 산출물:
  ...
  - 전체 노드가 동일한 AgentState 키를 참조할 수 있는 타입 일관성 확보
