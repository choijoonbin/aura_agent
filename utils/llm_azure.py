    ...
    try:
        ...
        for item in parsed.get("entities") or []:
            ...
            if isinstance(raw_bbox, list) and len(raw_bbox) >= 4:
                ymin = _clamp_bbox_value(raw_bbox[0], "ymin")
                xmin = _clamp_bbox_value(raw_bbox[1], "xmin")
                ymax = _clamp_bbox_value(raw_bbox[2], "ymax")
                xmax = _clamp_bbox_value(raw_bbox[3], "xmax")
                # 좌표 역전 보정
                if ymin > ymax:
                    ymin, ymax = ymax, ymin
                if xmin > xmax:
