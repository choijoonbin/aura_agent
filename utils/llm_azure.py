...
2. date_occurrence: "거래일자", "결제일시", "거래일" 레이블 행 → 날짜 값 (YYYY-MM-DD 변환)
3. time_occurrence: "거래시간", "결제시간", "거래일시" 레이블 행 → 시간 값 (HH:MM 변환)
   - "거래시간", "결제시간", "거래일시" 레이블이 있는 행에서만 찾으세요.
   - text: 24시간제 HH:MM 형식으로 반환 (예: "19:45")
   - AM/PM 표기는 24시간제로 변환 (예: "7:45 PM" → "19:45", "11:30 AM" → "11:30")
   - 초(seconds) 제거 후 HH:MM만 반환
4. amount_total: "합계금액", "총금액", "결제금액" 레이블 행 → 해당 라벨과 같은 줄의 최종 합계 금액
   - 중요: "공급가액", "부가가치세", "소계", "VAT" 행의 금액은 절대 선택하지 마세요.
   - "합계금액" 라벨 텍스트(key_index)와 같은 줄 또는 바로 다음에 오는 금액 텍스트를 선택하세요.
   - 영수증에 여러 금액이 있을 때 "합계금액" 라벨이 명시된 행의 금액이 최종 합계입니다.
